    ...
    <div>
        ...
        <div style="text-align: left">
            <el-form :model="inquireForm" ref="inquireForm" :inline="true">
                <el-form-item label="案件类型" prop="typea">
                    <el-cascader :options="cases" clearable v-model="inquireForm.typea"></el-cascader>
                </el-form-item>
                <el-form-item label="时间" prop="time">
                    <el-date-picker v-model="inquireForm.time" @change="getTime" type="date" placeholder="请选择日期"></el-date-picker>
                </el-form-item>
                <el-form-item label="姓名" prop="name">
                    <el-input v-model="inquireForm.name" placeholder="请输入报案人姓名"></el-input>
                </el-form-item>
                <el-form-item label="年级" prop="geade">
                    <el-select v-model="inquireForm.grade" clearable placeholder="请选择">
                        <el-option
                ...
                <el-form-item label="性别" prop="gender">
                    <el-select v-model="inquireForm.gender" clearable placeholder="请选择">
                        <el-option
                ...
                <el-form-item label="案发地点" prop="addressa">
                    <el-input v-model="inquireForm.addressa" placeholder="请输入案发地点"></el-input>
                    <!-- <el-cascader
                      :options="addresss"
                      v-model="inquireForm.addressa"></el-cascader> -->
                </el-form-item>
                <el-form-item label="财物价值">
                    <el-input style="width: 100px" placeholder="最低价" v-model="inquireForm.purchaseValue1"></el-input>
                    <span style="color: black">——</span>
                    <el-input style="width: 100px" placeholder="最高价" v-model="inquireForm.purchaseValue2"></el-input>
                </el-form-item>
                <el-form-item>
                    <el-button type="success"  @click="inquire('inquireForm')">查询</el-button>
                </el-form-item>
            </el-form>
        </div>
        <div style="margin-top: 50px">
            <el-table :data="tableData" border>
                ...
                <el-table-column label="联系方式" align="center" prop="contact" width="150"></el-table-column>
                ...
                <el-table-column label="操作" align="center" fixed="right" width="150">
                    <template slot-scope="scope">
                        ...
                        <el-button @click="pigeonhole(scope.row)" type="text" size="small">案件归档</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            caseType(){
                ...
                this.$get(`/api/findItem`).then(res=>{
                    ...
                    for(let i=0;i<res.body.caseType.length;i++){
                        ...
                        res.body.caseType[i].data.length && (typeA.children = [])
                        for(let e=0;e<res.body.caseType[i].data.length;e++){
                            ...
                            res.body.caseType[i].data[e].data.length && (typeB.children = [])
                    ...
                    this.cases = type;
                    this.occupations = res.body.occupation[0].tup;
                    for(let i=0;i<res.body.address.length;i++){
                        ...
                        for(let e=0;e<res.body.address[i].tup.length;e++){
                            ...
                            for(let j=0;j<res.body.address[i].tup[e].tup.length;j++){
                                ...
                                siteB.children.push(siteC)
                            }
                            siteA.children.push(siteB)
                        }
                        site.push(siteA);
                    }
                    this.addresss = site;
            ...
            inquire(formName){
                //查询
                this.$post(`/api/findsomea/${this.currentPage}/3`,{
                    typea:this.inquireForm.typea,
                    time:this.date,
                    name:this.inquireForm.name,
                    grade:this.inquireForm.grade,
                    gender:this.inquireForm.gender,
                    addressa:this.inquireForm.addressa,
                    purchaseValue1:this.inquireForm.purchaseValue1,
                    purchaseValue2:this.inquireForm.purchaseValue2
                }).then(res=>{
                    console.log(res);
//          this.$refs[formName].resetFields();
                    ...
                    for(let i=0;i<res.items.length;i++){
                        item = {
                            caseid:res.items[i].caseid,
                            name:res.items[i].name,
                            gender:res.items[i].gender,
                            age:res.items[i].age,
                            contact:res.items[i].contact,
                            spare1:res.items[i].spare1,
                            caseType:res.items[i].caseRecord.casetype,
                        };
                        data.push(item)
                    }
            ...
            load(){
                //获取案件列表
                this.$post(`/api/findsome/${this.currentPage}/3`).then(res=>{
                    console.log(res);
                    let data = [];
                    let item = {};
                    for(let i=0;i<res.items.length;i++){
